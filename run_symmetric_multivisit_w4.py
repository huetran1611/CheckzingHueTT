...
def main():
    exp.INSTANCES = sorted(glob.glob(r"test_data/special_data/symmetric_patch_multivisit/20/*.dat"))
    exp.THETAS = [1, 2]
    exp.RUNS = 3
    exp.DRONE_COUNTS = [1]
    exp.TRUCK_COUNTS = [2]
    exp.DRONE_CAPACITIES = [4]
    exp.DRONE_LIMIT_TIMES = [90]
    exp.WORKERS = 4
    exp.OUTPUT_CSV = os.path.join("result", f"multivisit_symmetric20_w4_{int(time.time())}.csv")
