...
def main() -> int:
    ...
    for row in rows:
        ...
        if rel is None:
            rel = read_instance_releases(inst)
            release_cache[inst] = rel
        row["best_solution"] = normalize_solution_text(str(row.get("best_solution") or ""), rel)
        row["best_multi_solution"] = normalize_solution_text(str(row.get("best_multi_solution") or ""), rel)
