...
def process_row(
    ...
    best_norm = normalize_solution_text(best_raw, releases)
    multi_norm = normalize_solution_text(multi_raw, releases)

    row["best_solution"] = best_norm
    row["best_multi_solution"] = multi_norm
    same_seed = bool(best_norm) and bool(multi_norm) and (best_norm == multi_norm)

    candidates_best: List[str] = [best_norm] if best_norm else []
    candidates_multi: List[str] = []
    if multi_norm:
        candidates_multi.append(multi_norm)

    base_best_metrics = None
    if best_norm:
        ok, _msg, metrics = validate_with_stats(solver_bin, n_truck, n_drone, instance, a, l, best_norm)
        if ok:
            base_best_metrics = metrics
    base_multi_metrics = None
    if multi_norm:
        okm, _msgm, metricsm = validate_with_stats(solver_bin, n_truck, n_drone, instance, a, l, multi_norm)
        if okm:
            base_multi_metrics = metricsm

    # Run LS from best_solution seed
    ...
    ok_ls1, msg_ls1, ls1_best, ls1_multi = run_local_search(
        ...
        n_drone,
        ls_time_limit_sec,
        ls_out_root / f"job_{job_tag}_best",
        instance,
        a,
        l,
        best_norm,
    ...
    # Run LS from best_multi seed only when different
    if multi_norm and not same_seed:
        ok_ls2, _msg_ls2, ls2_best, ls2_multi = run_local_search(
            solver_bin,
            n_truck,
            n_drone,
            ls_time_limit_sec,
            ls_out_root / f"job_{job_tag}_multi",
            instance,
            a,
            l,
            multi_norm,
        )
        if ok_ls2 and ls2_best:
            candidates_best.append(ls2_best)
        if ok_ls2 and ls2_multi:
            candidates_multi.append(ls2_multi)
    ...
    if not best_sol_final and best_norm and base_best_metrics is not None:
        best_sol_final = best_norm
    ...
    # Fallback requested:
    # if normalized best_solution and best_multi_solution both yield no valid solution,
...
def process_file(
    ...
    release_cache: Dict[str, List[int]] = {}
    # Preload releases before parallel section to avoid shared-write races.
    for row in rows:
    ...
    for i in range(len(rows)):
        ...
        row_new["f5_refine_status"] = "OK" if not reason else "WARN"
        row_new["f5_refine_reason"] = reason
        if reason:
            fail += 1
        rows[i] = row_new
...
def main() -> int:
    ...
    ap.add_argument("--ls-out-dir", default="result/Question4,14/ls_outputs_f5_row_jobs")
    args = ap.parse_args()
