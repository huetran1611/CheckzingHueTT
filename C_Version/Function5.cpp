...
static inline bool drone_package_eligible(const Params &p, int pkg) {
    return pkg > 0 && pkg < (int)p.customers.size() && p.customers[pkg].release > 0;
}
...
static bool reorder_drone_queue_by_truck_order(const Params &p, Solution &s);
static bool violates_drone_trip_order_conflict(const Solution &s);
...
static void apply_drone_local_search(const Params &p, Solution &s) {
    // Enforce Function5 invariant before LS operators: release=0 packages stay depot-loaded.
    normalize_after_transform(p, s);
    ...
    for (size_t t_idx = 0; t_idx < s.drone_queue.size(); ++t_idx) {
        ...
        for (int k = 0; k < K; ++k) {
            ...
            for (size_t pos = 1; pos + 1 < stops.size(); ++pos) {
                ...
                if (!drone_package_eligible(p, city)) continue;
...
static bool read_seed_solution_file(const Params &p, const std::string &path, Solution &sol, std::string &reason) {
    ...
    if (violates_drone_trip_order_conflict(cand)) {
        reason = "Seed solution invalid: conflicting drone trip order (cyclic waiting risk)";
        return false;
    }
...
static void repair_segment_precedence_same_truck(const Params &p, Solution &sol, int truck_id, int pos_lo, int pos_hi) {
    ...
    for (auto &trip : sol.drone_queue) {
        for (auto &ev : trip.events) {
            ...
            for (PackageId pkg : ev.packages) {
                ...
                if (pkg_pos >= rv_pos && drone_package_eligible(p, pkg)) kept.push_back(pkg);
    ...
    auto try_assign_pkg_same_truck = [&](int pkg) -> bool {
        ...
        if (!drone_package_eligible(p, pkg)) return false;
    ...
    for (auto &trip : sol.drone_queue) {
        for (auto &ev : trip.events) {
            ...
            for (PackageId pkg : ev.packages) {
                ...
                if (valid_owner && valid_order && drone_package_eligible(p, pkg)) kept.push_back(pkg);
                else displaced.insert(pkg);
            }
            ev.packages.swap(kept);
        }
...
static bool reorder_drone_queue_by_truck_order(const Params &p, Solution &s) {
    ...
    std::vector<DroneTrip> original = s.drone_queue;
    std::vector<DroneTrip> rebuilt;
    rebuilt.reserve(original.size());

    Solution work = s;
    work.drone_queue.clear();

    for (const auto &trip : original) {
        int ins = find_insert_pos_queue(p, work, trip);
        if (ins < 0) {
            // Keep trip as-is if we cannot find a legal insertion slot.
            work.drone_queue.push_back(trip);
        } else {
            work.drone_queue.insert(work.drone_queue.begin() + ins, trip);
        }
    }

    rebuilt = std::move(work.drone_queue);
    if (rebuilt.size() != original.size()) {
        s.drone_queue = std::move(rebuilt);
        return true;
    }
    bool changed = false;
    for (size_t i = 0; i < original.size(); ++i) {
        if (original[i].events.size() != rebuilt[i].events.size()) {
            changed = true;
            break;
        }
        for (size_t e = 0; e < original[i].events.size(); ++e) {
            const auto &a = original[i].events[e];
            const auto &b = rebuilt[i].events[e];
            if (a.truck_id != b.truck_id || a.rendezvous_customer != b.rendezvous_customer || a.packages != b.packages) {
                changed = true;
                break;
            }
        }
...
// Fast structural check for queue-order contradictions that can induce cyclic waiting.
// Return true if invalid conflicting order is detected.
static bool violates_drone_trip_order_conflict(const Solution &s) {
    const int K = (int)s.trucks.size();
    const int Q = (int)s.drone_queue.size();
    if (K <= 0 || Q <= 1) return false;

    std::unordered_map<int, int> owner;
    std::unordered_map<int, int> pos_on_truck;
    owner.reserve(1024);
    pos_on_truck.reserve(1024);
    for (int t = 0; t < K; ++t) {
        for (size_t i = 0; i < s.trucks[t].stops.size(); ++i) {
            int c = s.trucks[t].stops[i].customer;
            if (c <= 0) continue;
            if (!owner.count(c)) {
                owner[c] = t;
                pos_on_truck[c] = (int)i;
            }
        }
    }

    // Check queue monotonicity for each truck by earliest rendezvous in each trip.
    std::vector<int> last_pos(K, -1);
    std::vector<std::vector<int>> trip_pos(Q, std::vector<int>(K, -1));
    for (int q = 0; q < Q; ++q) {
        std::vector<int> first_pos(K, std::numeric_limits<int>::max());
        for (const auto &ev : s.drone_queue[q].events) {
            int t = ev.truck_id;
            if (t < 0 || t >= K) return true;
            auto it = owner.find(ev.rendezvous_customer);
            if (it == owner.end() || it->second != t) return true;
            int p = pos_on_truck[ev.rendezvous_customer];
            if (p < first_pos[t]) first_pos[t] = p;
            if (trip_pos[q][t] < 0 || p < trip_pos[q][t]) trip_pos[q][t] = p;
        }
        for (int t = 0; t < K; ++t) {
            if (first_pos[t] == std::numeric_limits<int>::max()) continue;
            if (last_pos[t] > first_pos[t]) return true;
            last_pos[t] = first_pos[t];
        }
    }

    // Pairwise cross-truck consistency: relative order between two trips must not flip across trucks.
    for (int i = 0; i < Q; ++i) {
        for (int j = i + 1; j < Q; ++j) {
            int dir = 0; // -1: i before j, +1: i after j
            for (int t = 0; t < K; ++t) {
                int pi = trip_pos[i][t], pj = trip_pos[j][t];
                if (pi < 0 || pj < 0 || pi == pj) continue;
                int cur = (pi < pj) ? -1 : +1;
                if (dir == 0) dir = cur;
                else if (dir != cur) return true;
            }
        }
    }
    return false;
...
static void normalize_after_transform(const Params &p, Solution &sol) {
    ...
    // owner[c] = truck id that serves customer c.
    std::vector<int> owner(n, -1);
    std::vector<int> pos_on_truck(n, -1);
    for (size_t t = 0; t < sol.trucks.size(); ++t) {
        for (size_t pos = 0; pos < sol.trucks[t].stops.size(); ++pos) {
            int c = sol.trucks[t].stops[pos].customer;
            if (c > 0 && c < n) {
                owner[c] = (int)t;
                pos_on_truck[c] = (int)pos;
            }
        }
    }

    // Remove invalid events/packages and enforce:
    // r_max(t) = max release of packages truck t carries from depot (not resupplied).
    // Any package with release <= r_max(t) must be depot-loaded, hence removed from drone resupply.
    while (true) {
        bool changed = false;

        // First pass: structural cleaning + cheap eligibility checks.
        for (auto &trip : sol.drone_queue) {
            std::vector<ResupplyEvent> kept_events;
            kept_events.reserve(trip.events.size());
            for (auto &ev : trip.events) {
                if (ev.truck_id < 0 || ev.truck_id >= (int)sol.trucks.size()) {
                    changed = true;
                    continue;
                }
                int rv = ev.rendezvous_customer;
                if (rv <= 0 || rv >= n || owner[rv] != ev.truck_id) {
                    changed = true;
                    continue;
                }
                int rv_pos = pos_on_truck[rv];
                if (rv_pos < 0) {
                    changed = true;
                    continue;
                }
                ...
                for (int x : ev.packages) {
                    if (!drone_package_eligible(p, x)) { changed = true; continue; }
                    if (owner[x] != ev.truck_id) { changed = true; continue; }
                    if (pos_on_truck[x] < rv_pos) { changed = true; continue; }
                    pk.push_back(x);
                }
                if (pk.empty()) {
                    changed = true;
        ...
        // Compute current resupplied set.
        std::vector<char> is_resup(n, 0);
        for (const auto &trip : sol.drone_queue) {
            for (const auto &ev : trip.events) {
                for (int pkg : ev.packages) {
                    if (pkg > 0 && pkg < n) is_resup[pkg] = 1;
                }
            }
        }

        // r_max per truck from depot-loaded customers.
        ...
        // Enforce r_max rule; may trigger cascading removals, so loop to fixpoint.
        bool removed_by_rmax = false;
        for (auto &trip : sol.drone_queue) {
            std::vector<ResupplyEvent> kept_events;
            kept_events.reserve(trip.events.size());
            for (auto &ev : trip.events) {
                ...
                for (int x : ev.packages) {
                    if (x <= 0 || x >= n) continue;
                    if (p.customers[x].release <= rmax[ev.truck_id]) {
        ...
        // Cleanup again after r_max pruning.
        {
            std::vector<DroneTrip> kept_trips;
            kept_trips.reserve(sol.drone_queue.size());
            for (auto &trip : sol.drone_queue) {
        ...
        if (!changed && !removed_by_rmax) break;
    }
...
static std::pair<bool,double> fitness_full(const Params &p, const Solution &sol, double *truck_sum_out) {
    if (violates_drone_trip_order_conflict(sol)) {
...
static bool relocate_package_first_improve(const Params &p, Solution &s) {
    ...
    for (int k = 0; k < p.n_truck; ++k) {
        ...
        for (size_t pos = 1; pos + 1 < s.trucks[k].stops.size(); ++pos) {
            ...
            if (!drone_package_eligible(p, pkg)) continue;
    ...
    for (const auto &c : cands) {
        ...
        for (size_t pos2 = pos_del+1; pos2 + 1 < s.trucks[k].stops.size(); ++pos2) {
            ...
            if (!drone_package_eligible(p, pk2)) continue;
...
static bool validate_solution(const Params &p, const Solution &sol, std::string &reason) {
    ...
    // 3d) Anti-cross consistency across shared trucks:
    // For any pair of trips, if they both rendezvous with multiple common trucks, the relative order
    // of those trips on each shared truck must be consistent (no "A before on truck X but after on truck Y").
    // This prevents cyclic inter-trip waiting patterns.
    std::vector<std::vector<int>> trip_pos_by_truck(sol.drone_queue.size(), std::vector<int>(sol.trucks.size(), -1));
    for (size_t q = 0; q < sol.drone_queue.size(); ++q) {
        for (const auto &ev : sol.drone_queue[q].events) {
            int t = ev.truck_id;
            if (t < 0 || t >= (int)sol.trucks.size()) continue;
            int pos = -1;
            ...
            if (pos < 0) continue;
            if (trip_pos_by_truck[q][t] == -1 || pos < trip_pos_by_truck[q][t]) {
                trip_pos_by_truck[q][t] = pos;
            }
        }
    }
    for (size_t i = 0; i < sol.drone_queue.size(); ++i) {
        for (size_t j = i + 1; j < sol.drone_queue.size(); ++j) {
            int dir = 0; // -1: trip i before trip j, +1: trip i after trip j
            int shared = 0;
            for (size_t t = 0; t < sol.trucks.size(); ++t) {
                int pi = trip_pos_by_truck[i][t];
                int pj = trip_pos_by_truck[j][t];
                ...
                if (dir == 0) dir = cur;
                    ...
                    reason = "Drone trips have inconsistent cross-truck order (potential cyclic waiting)";
                    return false;
                }
            }
            // One shared truck cannot create cross-truck inconsistency.
            (void)shared;
        }
    }
