...
static bool init_try_insert_multi_visit_trip(const Params &p, Solution &sol);
static bool init_force_multi_visit(const Params &p, Solution &sol);
...
static bool is_single_visit_with_drone_trip(const Solution &s) {
...
static void update_best_single_solution_if_better(const Params &p, const Solution &cand) {
    ...
    if (!g_has_best_single_solution) {
        g_has_best_single_solution = true;
        g_best_single_solution = cand;
        return;
    }
    double ts_best = 0.0;
    auto rb = fitness_full(p, g_best_single_solution, &ts_best);
    double ts_cur = 0.0;
    ...
    int mv_best = multi_visit_trip_count(g_best_single_solution);
    int trip_best = (int)g_best_single_solution.drone_queue.size();
    auto w_best = drone_wait_metrics(p, g_best_single_solution);
...
static void ats_full(const Params &p, Solution &s, int SEG = 4, double theta = 2.0, int DIV = 3) {
    ...
    std::cout << "[ATS] light-LS repeated-state skips: " << ls_light_skip_count << "\n";
	    ...
	    if (best_multi_fit < std::numeric_limits<double>::infinity()) {
            ...
            std::string mv_why;
            auto mvf = fitness_full(p, best_multi_sol, nullptr);
            if (validate_solution(p, best_multi_sol, mv_why) && mvf.first) {
	            g_has_best_multi_solution = true;
	            g_best_multi_solution = best_multi_sol;
            } else {
                std::cout << "[ATS] discard invalid best-multi candidate: " << mv_why << "\n";
...
// Generic init policy helper:
// - force selected packages to be resupplied in the initial solution
...
static void init_force_selected_resupply(const Params &p, Solution &sol, const std::vector<int> &selected, const std::string &label) {
    const int n = (int)p.customers.size();
    if (n <= 1 || sol.trucks.empty()) return;
    if (selected.empty()) return;

    std::vector<int> custs;
    custs.reserve(selected.size());
    for (int c : selected) if (c > 0 && c < n) custs.push_back(c);
    if (custs.empty()) return;
    std::sort(custs.begin(), custs.end(), [&](int a, int b){
        if (p.customers[a].release != p.customers[b].release) return p.customers[a].release > p.customers[b].release;
        return a < b;
    });
    custs.erase(std::unique(custs.begin(), custs.end()), custs.end());
    int target_cnt = (int)custs.size();
    ...
    std::cout << "[INIT] forced " << label << " resupply target " << target_cnt
              << " inserted " << inserted_pkg_cnt
              << " realized " << realized << "\n";
}

// Init policy:
// - force top ratio of highest-release packages to be resupplied in the initial solution.
static void init_force_top_release_resupply(const Params &p, Solution &sol, double ratio = 0.10) {
    ...
    init_force_selected_resupply(p, sol, custs, "top-release");
}

// Init policy:
// - force all packages with global max release date to be resupplied.
static void init_force_max_release_resupply(const Params &p, Solution &sol) {
    const int n = (int)p.customers.size();
    if (n <= 1 || sol.trucks.empty()) return;
    ...
    auto pick_rv_city = [&](int t, const std::vector<int> &pkgs) -> int {
        if (pkgs.empty()) return -1;
        int earliest_pos = std::numeric_limits<int>::max();
        for (int c : pkgs) earliest_pos = std::min(earliest_pos, owner_pos[c]);
        if (earliest_pos <= 0 || earliest_pos >= (int)sol.trucks[(size_t)t].stops.size()) return -1;
        int city = sol.trucks[(size_t)t].stops[(size_t)earliest_pos].customer;
        if (city_reachable(city)) return city;
        for (int pos = earliest_pos - 1; pos >= 1; --pos) {
            int c = sol.trucks[(size_t)t].stops[(size_t)pos].customer;
            if (city_reachable(c)) return c;
        }
        return -1;
    };

    auto sum_demand = [&](const std::vector<int> &pkgs) -> double {
        double s = 0.0;
        for (int c : pkgs) s += (double)p.customers[c].demand;
        return s;
    };

    std::vector<int> max_pkgs_all;
    std::vector<std::vector<int>> max_pkgs_by_truck((size_t)K);
    for (int c = 1; c < n; ++c) {
        if (p.customers[c].release != max_rel) continue;
        int t = owner_truck[c];
        if (t < 0 || t >= K) continue;
        if (owner_pos[c] <= 0) continue;
        if (p.customers[c].demand <= 0) continue;
        max_pkgs_by_truck[(size_t)t].push_back(c);
        max_pkgs_all.push_back(c);
    }
    if (max_pkgs_all.empty()) return;
    for (int t = 0; t < K; ++t) {
        auto &v = max_pkgs_by_truck[(size_t)t];
    ...
    // Clear old assignments of target max-release packages to avoid duplicates.
    std::vector<char> is_target(n, 0);
    for (int c : max_pkgs_all) is_target[c] = 1;
    for (auto &trip : sol.drone_queue) {
        for (auto &ev : trip.events) {
            std::vector<int> kept;
            kept.reserve(ev.packages.size());
    ...
    std::vector<int> active_trucks;
    for (int t = 0; t < K; ++t) if (!max_pkgs_by_truck[(size_t)t].empty()) active_trucks.push_back(t);
    if (active_trucks.empty()) return;

    auto build_single_trip = [&](int t, const std::vector<int> &pkgs, DroneTrip &out) -> bool {
        if (pkgs.empty()) return false;
        if (sum_demand(pkgs) > p.M_d + 1e-9) return false;
        int rv = pick_rv_city(t, pkgs);
        if (rv < 0) return false;
        out.events.clear();
        out.events.push_back(ResupplyEvent{rv, t, pkgs});
        return true;
    };

    auto build_multi_trip = [&](int ta, const std::vector<int> &pa, int tb, const std::vector<int> &pb, DroneTrip &out) -> bool {
        if (pa.empty() || pb.empty()) return false;
        if (sum_demand(pa) + sum_demand(pb) > p.M_d + 1e-9) return false;
        int rva = pick_rv_city(ta, pa), rvb = pick_rv_city(tb, pb);
        if (rva < 0 || rvb < 0) return false;
        out.events.clear();
        out.events.push_back(ResupplyEvent{rva, ta, pa});
        out.events.push_back(ResupplyEvent{rvb, tb, pb});
        return true;
    };

    int anchor_truck = active_trucks.front();
    double anchor_load = sum_demand(max_pkgs_by_truck[(size_t)anchor_truck]);
    for (int t : active_trucks) {
        double ld = sum_demand(max_pkgs_by_truck[(size_t)t]);
        if (ld < anchor_load) {
            anchor_load = ld;
            anchor_truck = t;
        }
    }
    std::vector<int> required_pkgs = max_pkgs_by_truck[(size_t)anchor_truck];

    auto all_required_resupplied = [&](const Solution &s, const std::vector<int> &req) -> bool {
        std::vector<char> is_resup(n, 0);
        for (const auto &trip : s.drone_queue)
            for (const auto &ev : trip.events)
                for (int x : ev.packages)
                    if (x > 0 && x < n) is_resup[x] = 1;
        for (int c : req) if (!is_resup[c]) return false;
        return true;
    };

    auto try_apply_pattern = [&](const std::vector<DroneTrip> &trips, const std::vector<int> &req, const std::string &label) -> bool {
        if (trips.empty()) return false;
        std::vector<int> ord(trips.size());
        std::iota(ord.begin(), ord.end(), 0);
        do {
            Solution trial = sol;
            bool ok = true;
            for (int idx : ord) {
                const DroneTrip &trip = trips[(size_t)idx];
                int ins = find_insert_pos_queue(p, trial, trip);
                if (ins < 0) { ok = false; break; }
                trial.drone_queue.insert(trial.drone_queue.begin() + ins, trip);
                if (!trip_endurance_optimistic(p, trial, trial.drone_queue[(size_t)ins])) { ok = false; break; }
                std::string reason;
                if (!validate_solution(p, trial, reason)) { ok = false; break; }
            }
            if (!ok) continue;
            if (!all_required_resupplied(trial, req)) continue;
            rebuild_loaded_from_drone_marks(p, trial);
            sol = std::move(trial);
            int realized = 0;
            std::vector<char> is_resup(n, 0);
            for (const auto &trip : sol.drone_queue)
                for (const auto &ev : trip.events)
                    for (int x : ev.packages)
                        if (x > 0 && x < n) is_resup[x] = 1;
            for (int c : req) if (is_resup[c]) realized++;
            std::cout << "[INIT] forced max-release(" << label << ") anchor_truck " << anchor_truck
                      << " target " << (int)req.size()
                      << " realized " << realized
                      << " multi " << (has_multi_visit(sol) ? 1 : 0) << "\n";
            return true;
        } while (std::next_permutation(ord.begin(), ord.end()));
        return false;
    };

    // Structured strategy focuses on 2-truck patterns (as requested).
    if (active_trucks.size() >= 2) {
        int t0 = active_trucks[0], t1 = active_trucks[1];
        const auto &p0 = max_pkgs_by_truck[(size_t)t0];
        const auto &p1 = max_pkgs_by_truck[(size_t)t1];

        // 1) If one trip can carry all max-release packages of both trucks => one multi rendezvous trip.
        {
            DroneTrip tr;
            if (build_multi_trip(t0, p0, t1, p1, tr)) {
                if (try_apply_pattern({tr}, required_pkgs, "1_multi_full")) return;
            }
        }

        // 2) Otherwise, try 2 trips: 1 single + 1 multi.
        //    Multi carries one package from anchor truck + one package from the other truck.
        //    Single carries the remaining anchor packages.
        for (int pass = 0; pass < 2; ++pass) {
            int ts = (pass == 0 ? t0 : t1); // truck for single trip
            int tm = (pass == 0 ? t1 : t0); // truck dominated by multi trip
            const auto &ps = max_pkgs_by_truck[(size_t)ts];
            const auto &pm = max_pkgs_by_truck[(size_t)tm];
            if (ps.empty() || pm.empty()) continue;
            if (ts != anchor_truck) continue; // ensure we fully cover one chosen truck first

            std::vector<int> multi_s = {ps.front()};
            std::vector<int> single_s(ps.begin() + 1, ps.end());
            std::vector<int> multi_m = {pm.front()};
            if (single_s.empty()) continue; // would collapse to one multi (already tried)

            DroneTrip tr_multi, tr_single;
            if (build_multi_trip(ts, multi_s, tm, multi_m, tr_multi) &&
                build_single_trip(ts, single_s, tr_single)) {
                if (try_apply_pattern({tr_single, tr_multi}, required_pkgs, "2_trip_single_plus_multi")) return;
            }
        }

        // 3) Otherwise, try 3 trips: 2 single + 1 multi (each truck contributes one anchor package to multi).
        {
            if (p0.size() >= 2 && p1.size() >= 2) {
                std::vector<int> m0 = {p0.front()}, s0(p0.begin() + 1, p0.end());
                std::vector<int> m1 = {p1.front()}, s1(p1.begin() + 1, p1.end());
                DroneTrip tr_m, tr_s0, tr_s1;
                if (build_multi_trip(t0, m0, t1, m1, tr_m) &&
                    build_single_trip(t0, s0, tr_s0) &&
                    build_single_trip(t1, s1, tr_s1)) {
                    if (try_apply_pattern({tr_s0, tr_s1, tr_m}, required_pkgs, "3_trip_2single_1multi")) return;
                }
            }
        }
    }

    // Fallback to previous generic forcing if structured patterns cannot be inserted feasibly.
    init_force_selected_resupply(p, sol, max_pkgs_all, "max-release");
}

// Try to inject one explicit multi-visit trip that rendezvous with 2 different trucks.
// This is a lightweight init-time constructor to increase the chance of having multi-visit
// structure before LS starts.
static bool init_try_insert_multi_visit_trip(const Params &p, Solution &sol) {
    const int n = (int)p.customers.size();
    const int K = (int)sol.trucks.size();
    if (n <= 1 || K < 2) return false;
    ...
    std::vector<char> already_resup(n, 0);
    for (const auto &trip : sol.drone_queue) {
        for (const auto &ev : trip.events) {
            for (int pkg : ev.packages) {
                if (pkg > 0 && pkg < n) already_resup[pkg] = 1;
            }
        }
    }

    std::vector<int> pick_pkg(K, -1), pick_city(K, -1);
    for (int c = 1; c < n; ++c) {
        int t = owner_truck[c];
        if (t < 0 || t >= K) continue;
        if (owner_pos[c] <= 0) continue;
        if (already_resup[c]) continue;
        if (p.customers[c].release <= 0) continue;
        if ((double)p.customers[c].demand > p.M_d + 1e-9) continue;
        int city = sol.trucks[(size_t)t].stops[(size_t)owner_pos[c]].customer;
        if (city <= 0 || city >= n) continue;
        if (!p.reachable_mask.empty() && !p.reachable_mask[city]) continue;
        if (pick_pkg[t] < 0) {
            pick_pkg[t] = c;
            pick_city[t] = city;
    ...
    for (int ta = 0; ta < K; ++ta) {
        if (pick_pkg[ta] < 0) continue;
        for (int tb = ta + 1; tb < K; ++tb) {
            if (pick_pkg[tb] < 0) continue;
            const double da = (double)p.customers[pick_pkg[ta]].demand;
            const double db = (double)p.customers[pick_pkg[tb]].demand;
            if (da + db > p.M_d + 1e-9) continue;

            for (int ord = 0; ord < 2; ++ord) {
                DroneTrip trip;
                if (ord == 0) {
                    trip.events.push_back(ResupplyEvent{pick_city[ta], ta, {pick_pkg[ta]}});
                    trip.events.push_back(ResupplyEvent{pick_city[tb], tb, {pick_pkg[tb]}});
                } else {
                    trip.events.push_back(ResupplyEvent{pick_city[tb], tb, {pick_pkg[tb]}});
                    trip.events.push_back(ResupplyEvent{pick_city[ta], ta, {pick_pkg[ta]}});
                }
                ...
                Solution trial = sol;
                trial.drone_queue.insert(trial.drone_queue.begin() + ins, trip);
                if (!trip_endurance_optimistic(p, trial, trial.drone_queue[(size_t)ins])) continue;
                std::string reason;
                if (!validate_solution(p, trial, reason)) continue;
                if (!has_multi_visit(trial)) continue;
...
// Enforce multi-visit structure right after initialization (when feasible).
// Strategy:
// 1) Try explicit construction of a 2-truck rendezvous trip.
// 2) If still single-visit, run a short phase-2 operator pass to induce multi-visit.
static bool init_force_multi_visit(const Params &p, Solution &sol) {
    if (has_multi_visit(sol)) return true;

    if (init_try_insert_multi_visit_trip(p, sol) && has_multi_visit(sol)) {
        std::cout << "[INIT] multi-visit forced by explicit 2-truck trip insertion\n";
        return true;
    }

    int saved_phase = g_ls_phase;
    g_ls_phase = 2; // allow equal-makespan moves that improve multi-visit structure

    bool changed = false;
    for (int rep = 0; rep < 6 && !has_multi_visit(sol); ++rep) {
        bool step = false;
        if (insert_rendezvous_first_improve(p, sol)) { step = true; changed = true; }
        if (!has_multi_visit(sol) && merge_sync_singletons_relaxed_first_improve(p, sol)) { step = true; changed = true; }
        if (!has_multi_visit(sol) && relocate_sync_point_first_improve(p, sol)) { step = true; changed = true; }
        if (!has_multi_visit(sol) && reorder_events_in_trip_first_improve(p, sol)) { step = true; changed = true; }
        if (!has_multi_visit(sol) && relocate_package_first_improve(p, sol)) { step = true; changed = true; }
        if (!has_multi_visit(sol) && reorder_trip_first_improve(p, sol)) { step = true; changed = true; }
        if (!step) break;
    }

    g_ls_phase = saved_phase;

    if (has_multi_visit(sol)) {
        std::cout << "[INIT] multi-visit forced by init operator pass\n";
        return true;
    }
    if (changed) {
        std::cout << "[INIT] init operator pass changed solution but multi-visit still not feasible\n";
    } else {
        std::cout << "[INIT] could not force multi-visit at init (likely infeasible for this instance/config)\n";
    }
    return false;
}
...
int main(int argc, char** argv){
    ...
    std::cout << "Makespan: " << sim.makespan << " (truck max " << *max_element(sim.truck_return.begin(), sim.truck_return.end())
        ...
        auto solution_is_valid = [&](const Solution &cand) -> bool {
            std::string why;
            if (!validate_solution(p, cand, why)) return false;
            auto fr = fitness_full(p, cand, nullptr);
            return fr.first;
	    ...
	    if (solution_is_valid(sol)) {
	        ...
	        std::cout << "[SOL] best solution file: (invalid)\n";
	    }

		    if (g_has_best_multi_solution && solution_is_valid(g_best_multi_solution)) {
	        std::cout.setf(std::ios::fixed);
	        std::cout << std::setprecision(12);
            auto fm = fitness_full(p, g_best_multi_solution, nullptr);
	        std::cout << "[SOL] best multi-visit makespan: " << fm.second << "\n";
	        ...
	        if (g_has_best_single_solution && solution_is_valid(g_best_single_solution)) {
	            std::cout.setf(std::ios::fixed);
	            std::cout << std::setprecision(12);
                auto fs = fitness_full(p, g_best_single_solution, nullptr);
	            std::cout << "[SOL] best single-visit makespan: " << fs.second << "\n";
	            std::cout << "[SOL] best single-visit solution detail:\n";
	            print_solution_compact(std::cout, g_best_single_solution, p);
	            std::string best_single_path = write_solution_json(g_best_single_solution, p, "best_single");
	            if (!best_single_path.empty()) {
	                std::cout << "[SOL] best single-visit solution file: " << best_single_path << "\n";
            } else {
                std::cout << "[SOL] best single-visit solution file: (write failed)\n";
            }
        } else {
